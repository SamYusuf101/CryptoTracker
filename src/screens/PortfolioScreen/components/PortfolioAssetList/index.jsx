...
const PortfolioAssetList = () => {
    ...
    const getCurrentBalance = () => assets.reduce((total, currentAsset) => 
    total + currentAsset.currentPrice * currentAsset.quantityBought, 0)
    
    const getCurrentValueChange = () => {
        const currentBalance = getCurrentBalance()
        const boughtBalance = assets.reduce((total, currentAsset)=>
        total + currentAsset.priceBought * currentAsset.quantityBought, 0
        );
        return (currentBalance - boughtBalance).toFixed(2)
    };

    const getCurrentPercetageChange = () => {
        const currentBalance = getCurrentBalance()
        const boughtBalance = assets.reduce((total, currentAsset)=>
        total + currentAsset.priceBought * currentAsset.quantityBought, 0
        );
                    ...
                    <>
                        <View style={styles.BalanceContainer}>
                            <View>
                                ...
                                <Text style={styles.currentBalanveValue}>${getCurrentBalance().toFixed(2)}</Text>
                            ...
                            <View style ={{...styles.PriceChangePercentageContainer, backgroundColor: isChangePositive() ?'green' : 'red'}}>
                                ...
                                />
                                <Text style={{...styles.PercentageChange, color: isChangePositive() ?'white' : 'white'}}>
                                {getCurrentPercetageChange()}%</Text>
                            </View>
                        </View>  
